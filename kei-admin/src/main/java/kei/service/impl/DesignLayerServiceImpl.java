...
import kei.domain.extrusionmodel.*;
import kei.domain.layer.LayerFileInfo;
import kei.domain.policy.GeoPolicy;
import kei.geospatial.LayerStyleParser;
import kei.geospatial.Ogr2OgrExecute;
import kei.persistence.DesignLayerFileInfoMapper;
import kei.persistence.DesignLayerMapper;
import kei.security.Crypt;
import kei.service.DesignLayerService;
...
public class DesignLayerServiceImpl implements DesignLayerService {
    ...
    */
    ...
    public String getDesignLayerExtent(DesignLayer designLayer) {
        ...
        } else if(DesignLayer.DesignLayerType.LINE == DesignLayer.DesignLayerType.valueOf(designLayer.getDesignLayerGroupType().toUpperCase())) {
            extent = designLayerMapper.getDesignLayerLineExtent(designLayerId);
        }
    ...
    */
        ...
        if(!designLayerFileInfoList.isEmpty()) {
            ...
            if(isDesignLayerFileInfoExist) {
                ...
                if(DesignLayer.DesignLayerType.LAND == DesignLayer.DesignLayerType.valueOf(designLayer.getDesignLayerGroupType().toUpperCase())) {
                    ...
                    designLayerFileInfoMapper.updateLinePreDataDisable(designLayerId);
                }
            }
    ...
    public void insertShapeInfo(DesignLayer designLayer, List<DesignLayer> shapeInfoList) {
        ...
        if(DesignLayer.DesignLayerType.LAND == DesignLayer.DesignLayerType.valueOf(designLayer.getDesignLayerGroupType().toUpperCase())) {
            ...
            shapeInfoList.forEach(f -> {
                var designLayerLine = modelMapper.map(f, DesignLayerLine.class);
                designLayerLine.setDesignLayerId(designLayer.getDesignLayerId());
                designLayerLine.setDesignLayerGroupId(designLayer.getDesignLayerGroupId());
                designLayerLine.setCoordinate(Integer.valueOf(designLayer.getCoordinate().split(":")[1]));
                designLayerLine.setTheGeom(f.getTheGeom());
                designLayerLine.setBuildId(f.getShapeId());
                designLayerMapper.insertGeometryLine(designLayerLine);
            });
        }
    }
    ...
    public void exportOgr2Ogr(DesignLayerFileInfo designLayerFileInfo, DesignLayer designLayer) throws Exception {
        ...
        } else if(DesignLayer.DesignLayerType.LINE == DesignLayer.DesignLayerType.valueOf(designLayerGroupType)) {
            tableName = propertiesConfig.getDesignLayerLineTable();
        }
    ...
    */
    ...
    public int updateDesignLayerByDesignLayerFileInfoId(Long designLayerId, Long designLayerFileInfoTeamId, Long designLayerFileInfoId) {
        ...
        if(DesignLayer.DesignLayerType.LAND == DesignLayer.DesignLayerType.valueOf(designLayer.getDesignLayerGroupType().toUpperCase())) {
            ...
            designLayerFileInfoMapper.updateLinePreDataDisable(designLayerId);
            designLayerFileInfoMapper.updateLineStatus(fileVersion);
        }
        ...
        try {
            ...
            if(DesignLayer.AttributeType.CSV == DesignLayer.AttributeType.valueOf(extension)) {
                ...
                for (CSVRecord record : records) {
                    ...
                    } else if(DesignLayer.DesignLayerType.LINE == DesignLayer.DesignLayerType.valueOf(type.toUpperCase())) {
                        DesignLayerLine designLayerLine = DesignLayerLine.builder()
                                .buildId(Long.valueOf(record.get(0)))
                                .buildMaximumFloors(Integer.valueOf(record.get(1)))
                                .build();
                        designLayerMapper.updateDesignLayerLineAttributes(designLayerLine);
                    }
                }
            } else {
                // TODO : xlsx, xls
	...
	public void rollbackDesignLayer(DesignLayer designLayer, boolean isDesignLayerFileInfoExist, DesignLayerFileInfo designLayerFileInfo, Long deleteDesignLayerFileInfoTeamId) {
		...
		if (isDesignLayerFileInfoExist) {
            ...
            if(DesignLayer.DesignLayerType.LAND == DesignLayer.DesignLayerType.valueOf(designLayer.getDesignLayerGroupType().toUpperCase())) {
                ...
                designLayerFileInfoMapper.updateLinePreDataDisable(designLayer.getDesignLayerId());
                designLayerFileInfoMapper.updateLineStatus(fileVersion);
                designLayerMapper.deleteGeometryBuildingHeight(map);
            }
	...
	public int deleteDesignLayer(Long designLayerId) {
		...
		if(!designLayerFilePath.isEmpty()) {
            ...
            if(DesignLayer.DesignLayerType.LAND == DesignLayer.DesignLayerType.valueOf(designLayer.getDesignLayerGroupType().toUpperCase())) {
                ...
                designLayerMapper.deleteGeometryLine(map);
            }
		}
