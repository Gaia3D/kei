...
DesignLayerObj.prototype.setSelectionInteraction = function(onOff){
    if(onOff){
    	...
    	/**
    	 * @param {string} dataType Mago3D.DataType을 이용해 타겟 설정 F4D, NATIVE, ALL, OBJECT 4가지 모드 존재
    	 */
        Ppmap.getManager().defaultSelectInteraction.setTargetType(Mago3D.DataType.NATIVE);
        Ppmap.getManager().defaultSelectInteraction.setActive(onOff);
        
        /**
    	 * @param {string} dataType Mago3D.DataType을 이용해 타겟 설정 F4D, NATIVE, ALL, OBJECT 4가지 모드 존재
    	 * @param {function} filterFunction 필터링 펑션
    	 */
        Ppmap.getManager().defaultSelectInteraction.setFilter(Mago3D.DataType.NATIVE, function(m) {
        	return !m.hasOwnProperty('designLayerId');
        });
        Ppmap.getManager().isCameraMoved = true;
    }else{
        Ppmap.getManager().defaultSelectInteraction.setActive(onOff);
        Ppmap.getManager().defaultSelectInteraction.setFilter(Mago3D.DataType.NATIVE, undefined);
...
DesignLayerObj.prototype.toggleExtrusionBuilding = function(d, isShow){
    ...
    let _height = function(data, entity){
        ...
        || DesignLayerObj.GroupType.LINE['text'] == data.designLayerGroupType){
    ...
    let extrusionClass = (d.data.designLayerGroupType === DesignLayerObj.GroupType.LINE['text']) ?  Mago3D.ExtrusionWall: Mago3D.ExtrusionBuilding;
    ...
    this.getFeatures({ 'typeNames': layerName, 'cql_filter': currentCqlFilter }, function (e) {
        ...
        for (let i = 0; i < entities.length; i++) {
            ...
            if(extrusionClass === Mago3D.ExtrusionWall) {
            	positions = entity.polyline.positions.getValue();
            	
            	option.colorsArray = [new Mago3D.Color(0.0,1.0,0,0.7), new Mago3D.Color(1.0,1.0,0,0.7), new Mago3D.Color(1.0,0,0,0.7)]
            	option.segmentCount = 2;
            } else {
            	positions = entity.polygon.hierarchy.getValue().positions;
                
                option.color = new Mago3D.Color.fromHexCode(_color(d.data, entity));
            	option.color.a = 0.5;
            	option.wireframeColor4 = option.color;
            }
            option.heightReference = Mago3D.HeightReference.CLAMP_TO_GROUND;
            
            let model = extrusionClass.makeExtrusionBuildingByCartesian3Array(positions.reverse(), h, option);
            model.type = d.data.designLayerGroupType;
            model.layerId = entity.id;
            model.designLayerId = d.data.designLayerId;
            
            if(extrusionClass === Mago3D.ExtrusionWall) {
            	Ppmap.getManager().effectsManager.addEffect(model._guid, new Mago3D.Effect({
    				effectType      : "fadeIn",
    				durationSeconds : 2.0
    			}));
            	Ppmap.getManager().effectsManager.addEffect(model._guid, new Mago3D.Effect({
             ...
             * magoManager에 속한 modeler 인스턴스의 addObject 메소드를 통해 모델 등록, 뒤의 숫자는 데이터가 표출되는 최소 레벨을 의미. 숫자가 낮을수록 멀리서 보임
             */
            Ppmap.getManager().modeler.addObject(model, 12);
            
            /**
             * 필지 폴리곤 정보로 건물에 제한 정보 설정
             */ 
            if(!(extrusionClass instanceof Mago3D.ExtrusionWall.constructor) && entity.properties.landuse_zoning && entity.properties.landuse_zoning.getValue().indexOf('공동주택') > -1) {
...
DesignLayerObj.prototype.offLimitInfo = function() {
	Ppmap.getManager().modeler.objectsArray.forEach(function(building) {
		if(building instanceof Mago3D.ExtrusionBuilding) {
...
DesignLayerObj.prototype.extrusionModelWMSLabelToggle = function(model, isShow){
    let url = [KEI.policy.geoserverDataUrl, KEI.policy.geoserverDataStore, model.ogctype].join('/');
    //
...
DesignLayerObj.prototype.offExtrusionModel = function(designLayerId){
    ...
    for(let i=0; i<models.length; i++){
        ...
        if(building.designLayerId == designLayerId || building.layerId == designLayerId) {
            if(building instanceof Mago3D.ExtrusionWall) {
            	Ppmap.getManager().effectsManager.addEffect(building._guid, new Mago3D.Effect({
    				effectType      : "zBounceLinearReverse",
    				durationSeconds : 0.9
    			}));
            	Ppmap.getManager().effectsManager.addEffect(building._guid, new Mago3D.Effect({
    				effectType      : "fadeOut",
    				durationSeconds : 1.0,
    				complete : function(magoManager) {
    					building.attributes.isVisible = false;
    					magoManager.modeler.removeObject(building);
    				}
    			}));
            } else {
...
DesignLayerObj.prototype.getFeatures = function(option, callbackFn){
    let opt = Pp.extend({'url':[KEI.policy.geoserverDataUrl, KEI.geoserverDataStore, 'wfs'].join('/')}, option);
