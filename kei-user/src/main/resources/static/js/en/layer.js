...
$(document).ready(function (){
	...
	$("#layerContent").on("click", '.layerGroup', function(e){
		...
		if($(this).text() === "OFF") {
			...
			KEI.map.addGroupLayer(Number(layerGroupId));
		} else {
			$(this).text("OFF");
			$('.nodepth').each(function(e){
				if(layerGroupId === $(this).attr("data-ancestor")) {
			...
			KEI.map.removeGroupLayer(Number(layerGroupId));
		}
	});
...
function layerOnOff(obj) {
	...
	if(flag) {
		KEI.map.addLayer(layerKey);
	} else {
		KEI.map.removeLayer(layerKey);
	}
}
...
function turnOnAllLayer() {
	...
	$('.nodepth').addClass("on");
	$('.layerGroup').text("ON");
	KEI.map.removeAllLayers();
	KEI.map.initLayer(true);
}
//레이어 전체 끄기
