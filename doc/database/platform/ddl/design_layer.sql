...
drop table if exists design_layer_line cascade;
...
comment on column design_layer_group.design_layer_group_type is 'design layer 그룹 분류. land : 땅, building : 빌딩, line : 건축물 배치';
...
comment on column design_layer.design_layer_type is 'design layer 분류. land : 땅, building : 빌딩, line : 건축물 배치';
...
comment on column design_layer_building_unit.design_layer_id is '디자인 레이어 고유번호';
...
-- design layer line
create table design_layer_line (
	design_layer_line_id                        bigint,
	design_layer_id					            bigint,
	design_layer_group_id			            integer,
	build_id                                    bigint,
	build_maximum_floors                        integer,
	...
	update_date					                timestamp with time zone,
	insert_date					                timestamp with time zone 		default now(),
	constraint design_layer_line_pk 		primary key (design_layer_line_id)
);

comment on table design_layer_line is '디자인 레이어 라인';
comment on column design_layer_line.design_layer_line_id is 'design layer 라인 고유번호';
comment on column design_layer_line.design_layer_id is 'design layer 고유번호';
comment on column design_layer_line.design_layer_group_id is 'design layer 그룹 고유번호';
comment on column design_layer_line.build_id is 'shape 파일 고유번호';
comment on column design_layer_line.build_maximum_floors is '최고층수';
comment on column design_layer_line.properties is '필수 컬럼 외 속성 값등을 key, value 형태로 저장하기 위함';
comment on column design_layer_line.the_geom is 'geometry';
comment on column design_layer_line.enable_yn is '활성화 유무';
comment on column design_layer_line.version_id is 'shape 버전 관리를 위한 번호';
comment on column design_layer_line.update_date is '수정일';
comment on column design_layer_line.insert_date is '등록일';
